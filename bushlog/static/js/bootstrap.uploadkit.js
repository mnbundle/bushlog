...
var UploadKit = function(input) {
	...
	var fileResize = this.fileResize = $input.data('fileResize') || this.fileResize;
	...
	var infoHtml = (isMultiple) ?
		'<h2>Browse for files to upload or drag and drop them here</h2>' :
		'<h2>Browse for file to upload or drag and drop here</h2>';
	...
	var $thead = this.$thead = $('<thead/>').html('<tr><th/><th/><th>Name</th><th>Size</th><th>Progress</th></tr>').appendTo($table);
	...
	var uploader = this.uploader = new plupload.Uploader({
		...
		flash_swf_url: '/static/js/assets/plupload.flash.swf',
		silverlight_xap_url: baseUrl + '/static/js/assets/plupload.silverlight.xap',
		filters: [
